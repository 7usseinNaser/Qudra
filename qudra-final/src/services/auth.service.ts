...
import { apiGet, apiPost, clearToken, setToken, getToken, type ApiErrorData } from './api/client';
...
export const AuthService = {
  ...
  async verifyEmailCode(code: string): Promise<boolean> {
    // BACKEND BLOCKED: no /api/v1/auth/verify-email endpoint exists yet
    // Mark locally only as a UI placeholder — this is NOT real verification
    if (code.length >= 4) {
