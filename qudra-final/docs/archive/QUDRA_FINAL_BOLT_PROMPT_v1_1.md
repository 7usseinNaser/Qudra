...
## الإصدار النهائي التنفيذي — v1.1 (مُصحَّح)
## الهدف: بناء B0 من الـprototype بأمانة، مع أساس قابل للتوسع

> **ملاحظة تصحيح النسخة:** هذه نسخة معدّلة عن مسودة سابقة. التعديلات: (1) شرط تنفيذ فعلي واضح لـSkill Discovery بدل افتراض توفر Web Access، (2) إزالة تناقض داخلي في قسم الخطوط بحيث لا يُفترض أي اسم خط دون تحقق من الكود الفعلي، (3) ترقيم كل بند في Quality Gate بمعرّف QG-XX قابل للتتبع، (4) إضافة قسم 0.5 لسد الفجوة الزمنية قبل قراءة الملفات الرسمية.
...
**تحقق أولاً هل تملك فعلياً أداة تصفح/جلب صفحات ويب (Web Access/Browsing tool) في بيئة التنفيذ الحالية.**

```text
IF (Web Access tool متاحة فعلياً):
    → افتح الروابط أعلاه فعلياً.
    → اقرأ محتواها الحقيقي.
    → قيّم واختر.
ELSE:
    → لا تتظاهر بأنك بحثت أو فتحت رابطاً.
    → لا تخترع اسم Skill أو محتوى Skill غير موجود لديك فعلياً.
    → انتقل مباشرة لاستخدام `SKILLS.md` المحلي كمرجع كافٍ ونهائي.
    → صرّح في تقرير المهمة: "Web Access غير متاحة — تم الاعتماد على SKILLS.md المحلي."
```

**هذا الشرط غير قابل للتجاوز.** الادّعاء بتنفيذ بحث لم يحدث فعلياً هو انتهاك مباشر لقاعدة "NO FALSE COMPLETION" (قسم 39).
